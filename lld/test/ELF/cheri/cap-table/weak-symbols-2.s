...
# RUN: llvm-readobj --dyn-relocations --symbols %t.exe | FileCheck %s --check-prefixes STATIC-COMMON,UNDEFINED
# STATIC-COMMON:       Dynamic Relocations {
# STATIC-COMMON-NEXT:  }
# STATIC-COMMON:       Symbol {
# STATIC-COMMON:         Name: weak_symbol@CAPTABLE (
# STATIC-COMMON-NEXT:    Value: 0x30{{.+}}
...
# RUN: llvm-readobj --dyn-relocations --symbols %t.exe | FileCheck %s --check-prefixes STATIC-COMMON,ABSOLUTE '-D$ABS_VALUE=0x40'
# ABSOLUTE:       Symbol {
...
# RUN: llvm-readobj --dyn-relocations --symbols %t.exe | FileCheck %s --check-prefixes STATIC-COMMON,ABSOLUTE '-D$ABS_VALUE=0x50'
...
# RUN: ld.lld --fatal-warnings -o %t.exe %t.o %t2.o
# RUN: llvm-readobj --dyn-relocations --symbols %t.exe | FileCheck %s --check-prefixes STATIC-COMMON,ABSOLUTE '-D$ABS_VALUE=0x60'
...
# RUN: ld.lld --fatal-warnings -o %t.exe %t.o %t2.o
# RUN: llvm-readobj --dyn-relocations --symbols %t.exe | FileCheck %s --check-prefixes STATIC-COMMON,RESOLVED '-D$RESOLVED_SIZE=8'
# RESOLVED:       Symbol {
# RESOLVED:         Name: weak_symbol (
# RESOLVED-NEXT:    Value: 0x40{{.+}}
...
# RUN: llvm-readobj --dyn-relocations --symbols %t.exe | FileCheck %s --check-prefixes STATIC-COMMON,RESOLVED '-D$RESOLVED_SIZE=0'
