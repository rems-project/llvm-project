...
template <class ELFT> void elf::createSyntheticSections() {
  ...
  if (config->emachine == EM_AARCH64 &&
    ...
    in.relaDyn = std::make_unique<RelocationSection<ELFT>>(relaDynName, /*sort=*/false);
  } else if (config->androidPackDynRelocs)
    in.relaDyn = std::make_unique<AndroidPackedRelocationSection<ELFT>>(relaDynName);
  else
    in.relaDyn = std::make_unique<RelocationSection<ELFT>>(relaDynName, config->zCombreloc);
  add(*in.relaDyn);
