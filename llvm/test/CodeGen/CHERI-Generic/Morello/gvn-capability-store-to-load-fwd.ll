...
; RUN: opt -mtriple=aarch64 --relocation-model=pic -target-abi purecap -mattr=+morello,+c64 -S -memdep -basic-aa -gvn -o - %s | FileCheck %s
; RUN: opt -mtriple=aarch64 --relocation-model=pic -target-abi purecap -mattr=+morello,+c64 -S -memdep -basic-aa -gvn -o - %s | llc -mtriple=aarch64 --relocation-model=pic -target-abi purecap -mattr=+morello,+c64 -O0 -o - | FileCheck %s --check-prefix=ASM
