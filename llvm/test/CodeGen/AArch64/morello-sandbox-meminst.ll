; RUN: llc -march=arm64 -mattr=+c64,+morello -target-abi purecap -o - %s | FileCheck %s --check-prefix=ALL --check-prefix=CFUN
...
; RUN: llc -march=arm64 -mattr=+c64,+morello -target-abi purecap -cheri-no-pure-cap-libfunc -o - %s | FileCheck %s --check-prefix=ALL --check-prefix=NOCFUN
