...
void SelectionDAGBuilder::visitGetElementPtr(const User &I) {
  ...
  if (FatPointer) {
    ...
    if (!OrigN.getValueType().isVector() && VectorWidth > 0) {
      LLVMContext &Context = *DAG.getContext();
      EVT VT = EVT::getVectorVT(Context, OrigN.getValueType(), VectorWidth);
...
static bool getUniformBase(const Value *&Ptr, SDValue &Base, SDValue &Index,
  ...
  if (STy) {
    ...
    Scale = DAG.getTargetConstant(1, SDB->getCurSDLoc(), TLI.getPointerRangeTy(DL));
...
void SelectionDAGBuilder::lowerCallToExternalSymbol(const CallInst &I,
  ...
  SDValue Callee = DAG.getExternalSymbol(
      FunctionName,
      DAG.getTargetLoweringInfo().getPointerTy(DL, DL.getGlobalsAddressSpace()));
...
#include "llvm/IR/ConstrainedOps.def"
  ...
  case Intrinsic::ptrmask: {
    ...
    // FIXME: Don't hard-code AS0, support capabilities here and use it for
    // __builtin_align_down.
    EVT DestVT =
        EVT(DAG.getTargetLoweringInfo().getPointerTy(DAG.getDataLayout(), 0));
...
bool SelectionDAGBuilder::visitMemPCpyCall(const CallInst &I) {
  ...
  const bool MustPreserveCheriTags =
      I.preservesTags() && TLI.cheriCapabilityType().isValid();
  Attribute CopyType =
