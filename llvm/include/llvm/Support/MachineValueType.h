...
namespace llvm {
  ...
  public:
    enum SimpleValueType : uint8_t {
      ...
      iFATPTR64      = 172,  // 64-bit fat pointer type
      iFATPTR128     = 173, // 128-bit fat pointer type
      iFATPTR256     = 174, // 256-bit fat pointer type
      iFATPTR512     = 175, // 512-bit fat pointer type
      iFATPTRAny     = 176, // Generic fat pointer type (must be legalised to a
      // sized  version)
  ...
  public:
    ...
    static SmallVector<MVT, 0> fixedlen_vector_valuetypes() {
      // FIRST_FAT_POINTER_VECTOR_VALUETYPE is not contiguous with the other
      // vector types to avoid ridiculous merge conflicts. To work around this
      // we create a temporary SmallVector here
      // Note: llvm::concat<MVT>(....) does not compile due since mvt_iterator
      // interface is not compatible.
      SmallVector<MVT, 0> Result(mvt_range(
          MVT::FIRST_FIXEDLEN_VECTOR_VALUETYPE,
          (MVT::SimpleValueType)(MVT::LAST_FIXEDLEN_VECTOR_VALUETYPE + 1)));
      for (int i = MVT::FIRST_FAT_POINTER_VECTOR_VALUETYPE;
           i <= MVT::LAST_FAT_POINTER_VECTOR_VALUETYPE; i++) {
        Result.push_back(MVT((MVT::SimpleValueType)i));
      }
      return Result;
    }
