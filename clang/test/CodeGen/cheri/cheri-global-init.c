...
// RUN: %clang_cc1 %s -emit-llvm -triple aarch64-none-linux-gnu -target-feature +c64 -target-abi purecap -o - -no-cheri-linker |  %cheri_FileCheck --check-prefix=AARCH64-NO-LINKER %s
// RUN: %clang_cc1 %s -emit-llvm -triple aarch64-none-linux-gnu -target-feature +c64 -target-abi purecap -o - | %cheri_FileCheck --check-prefix=AARCH64-LINKER %s
// RUN: %clang_cc1 %s -emit-llvm -triple aarch64-none-linux-gnu -target-feature +c64 -target-abi purecap -o - -mllvm -cheri-cap-table-abi=pcrel | %cheri_FileCheck --check-prefix=AARCH64-LINKER %s
...
// AARCH64-NO-LINKER: @llvm.global_ctors = appending addrspace(200) global [1 x { i32, void () addrspace(200)*, i8 addrspace(200)* }]
// AARCH64-NO-LINKER-SAME: [{ i32, void () addrspace(200)*, i8 addrspace(200)* } { i32 65535,
// AARCH64-NO-LINKER-SAME:  void () addrspace(200)* @_GLOBAL__sub_I_cheri_global_init.c,
// AARCH64-NO-LINKER-SAME: i8 addrspace(200)* null }]
// AARCH64-NO-LINKER: __cxx_global_var_init
// AARCH64-LINKER: @b = addrspace(200) global [3 x i32 addrspace(200)*] [i32 addrspace(200)* bitcast (i8 addrspace(200)* getelementptr (i8, i8 addrspace(200)* bitcast ([5 x i32] addrspace(200)* @a to i8 addrspace(200)*), i64 8) to i32 addrspace(200)*), i32 addrspace(200)* bitcast (i8 addrspace(200)* getelementptr (i8, i8 addrspace(200)* bitcast ([5 x i32] addrspace(200)* @a to i8 addrspace(200)*), i64 4) to i32 addrspace(200)*), i32 addrspace(200)* getelementptr inbounds ([5 x i32], [5 x i32] addrspace(200)* @a, i32 0, i32 0)], align [[#CAP_SIZE]]
// AARCH64-LINKER-NOT: __cxx_global_var_init
