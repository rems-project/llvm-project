...
// RUN: %cheri_purecap_cc1 -emit-llvm -o - -O2 %s | FileCheck %s -check-prefixes CHECK
// Check that this doesn't crash when emitting assembly:
// RXUN: %cheri_purecap_cc1 -S -o - -O2 %s | FileCheck %s -check-prefixes ASM
...
// CHECK-CHERI256: @_thr_umutex_init.default_mtx = internal addrspace(200) constant { i32, i32, [2 x i32], i8 addrspace(200)*, i32, [2 x i32] } zeroinitializer, align 16

// CHECK-LABEL: @_thr_umutex_init(
// CHECK-NEXT:  entry:
// CHECK-NEXT:    [[TMP0:%.*]] = bitcast [[STRUCT_UMUTEX:%.*]] addrspace(200)* [[MTX:%.*]] to i8 addrspace(200)*
// CHECK-NEXT:    tail call void @llvm.memcpy.p200i8.p200i8.i64(i8 addrspace(200)* align 16 [[TMP0]], i8 addrspace(200)* align 16 bitcast ({{.+}} addrspace(200)* @_thr_umutex_init.default_mtx to i8 addrspace(200)*), i64 48, i1 true)
// CHECK-NEXT:    ret void
//
