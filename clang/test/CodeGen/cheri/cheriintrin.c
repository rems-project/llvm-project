// RUN: %cheri_cc1 %s -o - -emit-llvm -O1 -Weverything -Werror -Wno-declaration-after-statement -verify | FileCheck %s
// RUN: %cheri_purecap_cc1 %s -o - -emit-llvm -O1 -Weverything -Wno-declaration-after-statement -Werror -verify | FileCheck %s
...
// CHECK-NEXT:    [[TMP29:%.*]] = call i64 @llvm.cheri.cap.load.tags.i64.{{.*}}
