...
// RXUN: %cheri_purecap_cc1 -emit-obj -mrelocation-model pic -O2 -std=gnu99 -ftls-model=local-exec -x c -o - -S %s | FileCheck %s -check-prefix ASM-PURECAP
// RXUN: %cheri_cc1 -emit-obj -mrelocation-model pic -O2 -std=gnu99 -ftls-model=local-exec -x c -o - -S %s | FileCheck %s -check-prefix ASM-MIPS
