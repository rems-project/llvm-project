...
// RUN: %clang -fgnuc-version=4.2.1 -x c++ -target cheri -mabi=n64 -E -dM %s -mno-gpopt |  FileCheck -check-prefix=CHECK-CHERI-N64 %s
// RUN: %clang_cc1 -fgnuc-version=4.2.1 -x c++ -triple cheri -target-abi n64 -E -dM %s | FileCheck -check-prefix=CHECK-CHERI-N64 %s
// RUN: %clang -fgnuc-version=4.2.1 -x c++ -target cheri -mabi=n64 -fexceptions -E -dM %s -mno-gpopt |  FileCheck -check-prefixes=CHECK-CHERI-N64,CHECK-CHERI-N64-FEXCEPTIONS %s
