...
template <class T, T *__capability Func> // hybrid-note 3 {{template parameter is declared here}}
struct c_cap {
...
void instantiate_cap() {
  // The following (valid) expressions were previously rejected by CHERI Clang:
  test_overloaded_fn_cap<void(int)>(1); // hybrid-error{{no matching function for call to 'test_overloaded_fn_cap'}}
  c_cap<void(int), a>::function(1); // hybrid-error{{non-type template argument of type 'void (int)' cannot be converted to a value of type 'void (* __capability)(int)'}}
  test_overloaded_fn_cap<void(long)>(1); // hybrid-error{{no matching function for call to 'test_overloaded_fn_cap'}}
  c_cap<void(long), a>::function(1); // hybrid-error{{non-type template argument of type 'void (long)' cannot be converted to a value of type 'void (* __capability)(long)'}}
  // This one should always be an error:
