...
// RUN: %clang -target aarch64-none-elf -march=morello+c64 -mabi=purecap -### -c -o /dev/null %s -fstack-protector-strong 2>&1 | \
// RUN:    FileCheck %s --implicit-check-not="-stack-protector" --check-prefix PURECAP-SSP
// RUN: %clang -target aarch64-none-elf -march=morello+c64 -mabi=purecap -### -c -o /dev/null %s -fstack-protector-all 2>&1 | \
// RUN:    FileCheck %s --implicit-check-not="-stack-protector" --check-prefix PURECAP-SSP
// RUN: %clang -target aarch64-none-elf -march=morello+c64 -mabi=purecap -### -c -o /dev/null %s -fstack-protector 2>&1 | \
// RUN:    FileCheck %s --implicit-check-not="-stack-protector" --check-prefix PURECAP-SSP

// RUN: %clang -target aarch64-none-elf -march=morello+c64 -mabi=purecap -### -c -o /dev/null %s -fno-stack-protector 2>&1 | \
