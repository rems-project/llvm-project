...
// RUN: %cheri_cc1 -Wall -Wcheri-prototypes-strict -fsyntax-only %s -verify=expected,strict
...
static void fn() {
  // strict-note@-1 {{'fn' declared here}}
}

extern void take_fn(void (*p)(void));

void pass_fn() {
  take_fn(fn); // strict-warning{{converting function type without prototype 'void ()' to function pointer 'void (*)(void)' may cause incorrect parameters to be passed at run time}}
  // strict-note@-1{{Calling functions without prototypes is dangerous because on some architectures (e.g. pure-capability MIPS) integer and pointer arguments are passed in different registers. If the call includes a parameter that does not match the function definition, the function will read uninitialized values from the argument registers.}}
}
